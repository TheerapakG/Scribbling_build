...
!define MULTIUSER_INSTALLMODE_INSTDIR "Scribbling"
...
Section "!${PRODUCT_NAME}" sec_app
  ...
  File /r /x "tgraphics" /x ".git" /x "__pycache__" "pkgs\*.*"
  SetOutPath "$INSTDIR\pkgs\tgraphics"
  File /r /x "demo" /x ".git" /x "__pycache__" "pkgs\tgraphics\tgraphics\*.*"
  File "pkgs\tgraphics\LICENSE"
  File "pkgs\tgraphics\README.md"
  SetOutPath "$INSTDIR"
  ...
  ; Install directories
    ...
    File /r "Python\*.*"
    SetOutPath "$INSTDIR\other_dependencies"
    File /r "other_dependencies\*.*"
  ...
  SetOutPath "%HOMEDRIVE%\%HOMEPATH%"
    CreateShortCut "$SMPROGRAMS\Scribbling.lnk" "$INSTDIR\Python\python.exe" \
      '"$INSTDIR\run.py"' "$INSTDIR\glossyorb.ico"
  SetOutPath "$INSTDIR"
...
Section "Uninstall"
  ...
  IfFileExists "$INSTDIR\.sdata" 0 +2
  Delete "$INSTDIR\.sdata"
  IfFileExists "$INSTDIR\__pycache__" 0 +2
  ...
  ; Uninstall files
    ...
    Delete "$INSTDIR\deps.py"
    Delete "$INSTDIR\run.1.py"
  ; Uninstall directories
    RMDir /r "$INSTDIR\Python"
    RMDir /r "$INSTDIR\other_dependencies"
    RMDir /r "$INSTDIR\resources"

  ; Uninstall shortcuts
      Delete "$SMPROGRAMS\Scribbling.lnk"
  RMDir $INSTDIR
